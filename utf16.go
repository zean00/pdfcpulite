...
func IsUTF16BE(b []byte) (ok bool, err error) {
	...
	if len(b)%2 != 0 {
		return false, fmt.Errorf("DecodeUTF16String: UTF16 needs even number of bytes: %v\n", b)
	}
...
func decodeUTF16String(b []byte) (s string, err error) {
	...
	if !isUTF16BE {
		err = fmt.Errorf("decodeUTF16String: not UTF16BE: %v\n", b)
		return
	}
	...
	for i := 0; i < len(b); {
		...
		if i+2 >= len(b) {
			err = fmt.Errorf("decodeUTF16String: corrupt UTF16BE on unicode point 1: %v", b)
			return
		}
		...
		if val < 0xDC00 || val > 0xDFFF {
			err = fmt.Errorf("decodeUTF16String: corrupt UTF16BE on unicode point 2: %v", b)
			return
		}
