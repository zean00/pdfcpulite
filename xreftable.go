...
func (xRefTable *XRefTable) Free(objNr int) (*XRefTableEntry, error) {
	...
	if !found {
		return nil, fmt.Errorf("Free: object #%d not found.", objNr)
	}
...
func (xRefTable *XRefTable) InsertAndUseRecycled(xRefTableEntry XRefTableEntry) (objNr int, err error) {
	...
	fmt.Println("InsertAndUseRecycled: begin")
	...
	if *freeListHeadEntry.Offset == 0 {
		...
		fmt.Printf("InsertAndUseRecycled: end, new objNr=%d\n", objNr)
		return objNr, nil
	}
	...
	fmt.Printf("InsertAndUseRecycled: end, recycled objNr=%d\n", objNr)
...
func (xRefTable *XRefTable) EnsureValidFreeList() error {
	...
	fmt.Println("EnsureValidFreeList begin")
	...
	if len(m) == 0 {
		...
		fmt.Println("EnsureValidFreeList: empty free list.")
		return nil
	}
	...
	for f != 0 {
		...
		fmt.Printf("EnsureValidFreeList: validating obj #%d %v\n", f, m)
	...
	if len(m) == 0 {
		fmt.Println("EnsureValidFreeList: end, regular linked list")
		return nil
	}
	...
	fmt.Println("EnsureValidFreeList: end, linked list plus some dangling free objects.")
...
func (xRefTable *XRefTable) DeleteObjectGraph(o Object) error {
	...
	fmt.Println("DeleteObjectGraph: begin")
	...
	fmt.Println("DeleteObjectGraph: end")
	return nil
}
...
func (xRefTable *XRefTable) DeleteObject(objNr int) error {
	...
	fmt.Printf("DeleteObject: begin %d\n", objNr)
	...
	entry, found := xRefTable.FindTableEntryLight(objNr)
	if !found {
		return fmt.Errorf("pdfcpu: deleteObject: no entry for obj #%d\n", objNr)
	}

	if entry.Free {
		fmt.Printf("DeleteObject: end %d already free\n", objNr)
		return nil
	}
...
func (xRefTable *XRefTable) UndeleteObject(objectNumber int) error {
	...
	fmt.Printf("UndeleteObject: begin %d\n", objectNumber)
	...
	for *f.Offset != 0 {
		...
		if objNr == objectNumber {
			fmt.Printf("UndeleteObject end: undeleting obj#%d\n", objectNumber)
	...
	fmt.Printf("UndeleteObject: end: obj#%d not in free list.\n", objectNumber)
...
func (xRefTable *XRefTable) DereferenceStringOrHexLiteral(obj Object, sinceVersion Version, validate func(string) bool) (s string, err error) {
	...
	default:
		return "", fmt.Errorf("pdfcpu: dereferenceStringOrHexLiteral: wrong type <%v>", obj)
...
func (xRefTable *XRefTable) DereferenceText(o Object) (string, error) {
	...
	default:
		return s, fmt.Errorf("pdfcpu: textString: corrupt -  %v\n", obj)
	}
...
func (xRefTable *XRefTable) DereferenceArray(o Object) (Array, error) {
	...
	a, ok := o.(Array)
	if !ok {
		return nil, fmt.Errorf("pdfcpu: dereferenceArray: wrong type <%v>", o)
	}
...
func (xRefTable *XRefTable) DereferenceStreamDict(o Object) (*StreamDict, error) {
	...
	sd, ok := o.(StreamDict)
	if !ok {
		return nil, fmt.Errorf("pdfcpu: dereferenceStreamDict: wrong type <%v>", o)
	}
...
func (xRefTable *XRefTable) DereferenceDictEntry(d Dict, entryName string) (Object, error) {
	...
	if !found || o == nil {
		return nil, fmt.Errorf("pdfcpu: dict=%s entry=%s missing.", d, entryName)
	}
...
func (xRefTable *XRefTable) freeList(logStr []string) ([]string, error) {
	...
	for f != 0 {
		...
		fmt.Printf("freeList validating free object %d\n", f)
		...
		fmt.Printf("freeList: %s", s)

		f = next
	}

	fmt.Printf("freeList end")
...
func (xRefTable *XRefTable) bindNameTreeNode(name string, n *Node, root bool) error {
	...
	if n.leaf() {
		...
		dict.Update("Names", a)
		fmt.Printf("bound nametree node(leaf): %s/n", dict)
		return nil
	}
	...
	fmt.Printf("bound nametree node(intermediary): %s/n", dict)
...
func (xRefTable *XRefTable) RemoveNameTree(nameTreeName string) error {
	...
	fmt.Printf("Deleted Names from root: %s\n", rootDict)
...
func (xRefTable *XRefTable) RemoveCollection() error {
	...
	fmt.Printf("deleted collection from root: %s\n", rootDict)
...
func (xRefTable *XRefTable) processPageTree(root *IndirectRef, pAttrs *InheritedPageAttrs, p *int, page int) (Dict, error) {
	...
	for _, o := range kids {
		...
		ir, ok := o.(IndirectRef)
		if !ok {
			return nil, fmt.Errorf("pdfcpu: processPageTree: corrupt page node dict")
		}
...
func (xRefTable *XRefTable) collectMediaBoxesForPageTree(root *IndirectRef, inhMediaBox *Rectangle, mediaBoxes []*Rectangle, p *int) (*Rectangle, error) {
	...
	for _, o := range kids {
		...
		ir, ok := o.(IndirectRef)
		if !ok {
			return nil, fmt.Errorf("pdfcpu: collectMediaBoxesForPageTree: corrupt page node dict")
		}
...
func (xRefTable *XRefTable) insertIntoPageTree(root *IndirectRef, pAttrs *InheritedPageAttrs, p *int, selectedPages IntSet, before bool) (int, error) {
	...
	for _, o := range kids {
		...
		ir, ok := o.(IndirectRef)
		if !ok {
			return 0, fmt.Errorf("pdfcpu: insertIntoPageTree: corrupt page node dict")
		}
